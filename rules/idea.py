...
KEMENY_RULE_MAX_SUPPORTED_CANDIDATES_COUNT = 10


def idea_rule(profile: Profile, candidate: int) -> int:
    candidate_to_heuristic_score = dict()
    for c in profile.candidates:
        candidate_to_heuristic_score[c] = veto_rule(profile, c)
    ...
    if candidate in candidates_excluded_from_kemeny_rule:
        ...
        profile_with_best_candidates = Profile(pairs=pairs_with_best_candidates)
